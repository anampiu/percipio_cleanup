...
df.to_csv('./temp/bd_reordered_colums.csv', index=False)

print("BD report clean up is ongoing...")

#Filter for BD team courses
data = pd.read_csv('./temp/bd_reordered_colums.csv')
...
with pd.ExcelWriter('./temp/bd_remove_duplicates.xlsx') as writer:
    sheet_written = False
...
file_path = './temp/bd_remove_duplicates.xlsx'  # Path to your Excel file
...
print(f"BD cleaned data saved to '{output_file_path}'")